...
vintage.run = function () {
  ...
  vintage.hues = shuffle(vintage.hues); // vintage.hues.sort((a,b)=>a-b);
  // for (i=0;i<=vintage.numOfColors/4;i++){
  //     vintage.hues.unshift(vintage.hues.pop());
  // }

  vintage.saturations.sort();
  vintage.brightnesses.sort(function (a, b) {
    return b - a;
  });
  ...
  for (i = 0; i < vintage.numOfColors; i++) {
    var color = document.createElement('div'),
        hsb = {
      h: vintage.hues[i],
      s: vintage.saturations[i],
      b: vintage.brightnesses[i]
    },
        rgb = HSBToRGB(hsb); // console.log(hsb);

    color.classList.add('color');
    color.style.backgroundColor = "rgb(".concat(rgb.r, ", ").concat(rgb.g, ", ").concat(rgb.b, ")");
